...
public class Engine
{
	...
	private final TimeCounter logCollisions = new TimeCounter("COLLISIONS: ");
	private final TimeCounter logProcesses = new TimeCounter("PROCESSES:  ");
	private final TimeCounter logRender = new TimeCounter("RENDER:     ");
	private final TimeCounter logUpdateTotal = new TimeCounter("UPDATE:     ");

	public void update(float delta, GL10 screen)
	{
	...
	private void render(GL10 screen)
	{
		screen.glClear(GL10.GL_COLOR_BUFFER_BIT | GL10.GL_DEPTH_BUFFER_BIT);
		screen.glMatrixMode(GL10.GL_PROJECTION);
		screen.glLoadIdentity();
		screen.glOrthof(this.camera.x, this.camera.x + this.camera.width, this.camera.y, this.camera.y + this.camera.height, 1, -1);
		screen.glMatrixMode(GL10.GL_MODELVIEW);
		...
		renderProcesses(processesToRender, numberOfProcess, screen);
	}
	...
	private int addProcessesToRender(SparseArray<Process> list, SparseArray<List<Process>> processesToRender)
	{
		...
		float cameraRight = this.camera.x + this.camera.width;
		float cameraTop = this.camera.y + this.camera.height;
	...
	private void renderProcesses(SparseArray<List<Process>> processesToRender, int numberOfProcess, GL10 screen)
	{
		int zValue = 0;
		int rendered = 0;
		...
		while (rendered < numberOfProcess)
		{
			...
			if (listOfProcesses != null)
			{
				...
				{
					listOfProcesses.get(i).render(screen);
				}
			}
